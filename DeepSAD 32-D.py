...
class NeuralNet(nn.Module):
    ...
    def __init__(self, size):
        ...
        self.fc5 = nn.Linear(64, 32)  # Output 32 units
    ...
    def forward(self, x):
        ...
        x = self.m(self.fc4(x))
        x = self.m(self.fc5(x))
        return x  # 32-dimensional output
...
class NeuralNet_Decoder(nn.Module):
    ...
    def __init__(self, size):
        ...
        self.size = size  # Set size to an attribute
        # Create network layers
        self.fc1 = nn.Linear(32, 64)  # Input: 32 units from the encoder's output
    ...
    def forward(self, x):
        ...
        x = self.m(self.fc4(x))
        x = self.m(self.fc5(x))
        x = x.view(-1, self.size[0], self.size[1])  # Reconstruct matrix of original data dimensions
        return x
...
def init_c(model, train_loader, eps=0.1):
    ...
    n_samples = 0
    c = torch.zeros(32)  # 32-dimensional center
...
def train(model, train_loader, learning_rate, weight_decay, n_epochs, lr_milestones, gamma, eta, eps, reg=0.001):
    ...
    for epoch in range(n_epochs):
        ...
        for train_data, train_target in train_loader:
            loss = 0.0
            optimizer.zero_grad()
            outputs = model(train_data)  # Outputs are now 32-dimensional vectors

            # Calculating the distance between output and the hypersphere center
            Y = outputs - model.c  # Y is now a 32-dimensional vector
            dist = torch.sum(Y ** 2, dim=1)  # Sum squared differences along the dimension

            loss_d = 0
            if reg != 0:  # If we want to diversify
                C = torch.matmul(Y.T, Y)  # Gram Matrix for 32-dimensional output
                loss_d = -torch.log(torch.det(C) + 1e-7) + torch.trace(C)  # Diversity loss

            # Loss calculation
            if train_target == 0:
                losses = dist  # When target is 0 (normal), we use the distance
            else:
                losses = eta * ((dist + eps) ** train_target)  # When target is non-zero (anomalous)

            losses += reg * loss_d  # Add the regularization term
