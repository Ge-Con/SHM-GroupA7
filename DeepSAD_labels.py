...
def findAnomalies(model, data_loader, eps):
    model.eval()
    scores = []
    with torch.no_grad():
        for data, _ in data_loader:
            outputs = model.encoder(data)
            dist = torch.sum((outputs - model.encoder.c) ** 2, dim=1)
            scores.append(dist)
        return scores

def evalModel(model, data_loader, data_targets, eps):
    model.eval()
    scores = []
    preds = []
    with torch.no_grad():
        for data, _ in data_loader:
            ...
            dist = torch.sum((outputs - model.encoder.c) ** 2, dim=1)
            scores.append(dist)
            preds += [1 if d.item() > eps else 0 for d in dist]
    true_labels = data_targets.cpu().numpy()
    accuracy = sum(np.array(preds) == true_labels) / len(true_labels)
    return accuracy
...
def objective(values):
    ...
    n_epochs = values [2]
    lr_milestones = [int(values[3] * n_epochs)]
    gamma = values [4]
    eta = values [5]
    eps = values [6]
    reg = values [7]
    model = NeuralNet_Autoencoder(size=[4, 4])
    model = AE_train(model, train_loader, learning_rate, weight_decay, n_epochs, lr_milestones, gamma)
    model = train(model, train_loader, learning_rate, weight_decay, n_epochs, lr_milestones, gamma, eta, eps, reg)
    accuracy = evalModel(model, test_loader, test_targets, eps)
...
def DeepSAD_train_run(dir, frequency, filename):
    global train_loader, test_loader, test_targets  # Declare globals at the beginning

    file_path = os.path.join(dir, f"{filename}_{frequency}kHz.csv")
    ...
    space = [
        ...
        Real(1e-6, 1e-1, "log-uniform", name="weight_decay"),
        Integer(10, 100, name="n_epochs"),
        Real(0.0, 1.0, name="lr_milestones_ratio"),
        Real(0.1, 1.0, name="gamma"),
        Real(0.0, 1.0, name="eta"),
        Real(1e-6, 1e-1, "log-uniform", name="eps"),
        Real(0.0, 1.0, name="reg")
    ]
    train_data = df[df["label"] == 0].sample(frac=0.8, random_state=1)
    test_data = df.drop(train_data.index)
    train_targets = train_data.pop("label")
    test_targets = test_data.pop("label")
    ...
    model = NeuralNet_Autoencoder(size=size)
    model = AE_train(model, train_loader, best_params[0], best_params[1], best_params[2], [int(best_params[3] * best_params[2])], best_params[4])
    model = train(model, train_loader, best_params[0], best_params[1], best_params[2], [int(best_params[3] * best_params[2])], best_params[4], best_params[5], best_params[6], best_params[7])
    scores = findAnomalies(model, test_loader, best_params[6])
    return scores
