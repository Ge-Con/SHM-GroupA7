...
def store_hyperparameters(params, file, panel, freq, dir):
...
def train_vae(hidden_1, batch_size, learning_rate, epochs):
    ...
    global scaler
    global pca
    panels = ("PZT-FFT-HLB-L1-03", "PZT-FFT-HLB-L1-04", "PZT-FFT-HLB-L1-05", "PZT-FFT-HLB-L1-09", "PZT-FFT-HLB-L1-23")
    n_input = data.shape[1]  # Number of features
    hidden_2 = 1
    display = 50
    ...
    for j in tuple(x for x in panels if x != panel):
        graph_data = pd.read_csv(dir_root + "\concatenated_" + freq + "_kHz_" + j + "_" + file + ".csv", header=None).values.transpose()
...
def VAE(dir_root, file, opt = False):
    global data     #This seems an especially bad name for a global variable
    global test
    global valid
    global scaler
    global pca
    ...
    resdict = {}
    counter = 0
    health_indicators = np.empty((6, 5), dtype=object)
    if not opt:
    ...
    for panel in panels:
        for freq in freqs:
            ...
            for i in tuple(x for x in panels if x != panel):
                filename = os.path.join(dir_root, f"concatenated_{freq}_KHz_{i}_{file}.csv")
                filenames.append(filename)
            resdict[f"{panel}{freq}"] = []
            for j in range(1):
                counter += 1
                data, flags = mergedata(filenames)
                test_filename = os.path.join(dir_root, f"concatenated_{freq}_KHz_{panel}_{file}.csv")
                ...
                # Set hyperparameters and architecture details
                    ...
                    store_hyperparameters(hyperparameters, file, panel, freq, dir_root)

                else:
                    hyperparameters_str = hyperparameters_df.loc[freq, panel]
                    hyperparameters = eval(hyperparameters_str)
                    health_indicators[freq][panel] = train_vae(hyperparameters[0], hyperparameters[1], hyperparameters[2], hyperparameters[3], panel)
